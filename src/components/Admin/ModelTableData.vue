<template>
  <b-row class="justify-content-md-center">
    <b-modal ref="my-modal" size="lg" hide-footer title="Add Model">
      <div >      
        <b-form >
          <b-form-group
            id="input-group-1"
          ...
          <b-form-group
            ...
            label-for="input-2"
          >
            <b-form-select v-model="form.id_brand" :options="this.groups"></b-form-select>
...
export default {
  ...
  methods: {
    onEdit(item_info) {
      ...
      this.form.id_brand=item_info.id_brand
    ...
    onArchive(item) {
      console.log(item)
      this.service.archive("model",item.id).then(() => {
    ...
    row_clicked: function (item_id) {
      // console.log(object.id_brand_item.id)
    ...
    async get_data(page = 1, limit = 12, sort = 'id', q = '', order = 'desc') {
      ...
      let response = (await this.service.get_all_items("model",params)).data
      var items=response.items
      for (var key in items) {
        var item = items[key];
        var brand_info=item["brand_info"]
        console.log(brand_info)
        brand_info === null ? item["brand"]="" : item["brand"]=brand_info.title
      }
      this.rows = items
    ...
    onUpdateItem(){
      ...
      this.service.update("model",this.form).then(response => {
          console.log('group updated', response)
          this.$bvToast.toast(`Model has been successfully updated`, {
            title:'Model',
            toaster:'b-toaster-top-center',
            autoHideDelay: 3000
          })
          ...
          console.log('model creation error', error.response)
    ...
    onSubmit() {
        alert(JSON.stringify(this.form))   
        if (!this.form.id_brand){
            this.error.category="Please select brand"
            return
        ...
        this.service.create("model",this.form).then(response => {
          console.log('model created', response)
          this.$bvToast.toast(`Model has been successfully created`, {
            title:'Model',
            toaster:'b-toaster-top-center',
            autoHideDelay: 3000
          })
  ...
  async created() {
    let response = (await this.service.get_all_items("model")).data
    ...
    .filter(key =>
        ['archived', 'id_brand', 'brand_info', 'images', 'created_at', 'last_updated'].indexOf(key) === -1)
    .map(key => {
    ...
    let params = {page: 1, limit: 1000, sort: "id", order: "asc"}
    let resGroup = (await this.service.get_all_items("brand",params)).data
