      ...
      <form class="validated" novalidate v-if="field_data" :class="[validated?'was-validated':'need-validation']">
        ...
        <b-row>
          <b-col cols="4"  class="mb-4">
            <label for="brand" style="text-transform: capitalize">Brand</label>
            <select class="form-control" v-model="selectedBrand"  @change="onChangeBrand(selectedBrand)" required>
              <option v-for="brand in this.master_data.brands" v-bind:value="{ id: brand.id, text: brand.title }" :key="brand.id">
                {{ brand.title }}
              </option>
            </select>  
          </b-col>
          <b-col cols="4"  class="mb-4">
            <label for="brand" style="text-transform: capitalize">Model</label>
            <select class="form-control" v-model="selectedModel" required>
              <option v-for="model in this.master_data.modelsBasedOnBrand" v-bind:value="{ id: model.id, text: model.title }" :key="model.id">
                {{ model.title }}
              </option>
            </select>  
          </b-col>
          ...
          <b-col cols="4"  class="mb-4">
            <label for="brand" style="text-transform: capitalize">Type</label>
            <select class="form-control" v-model="selectedType">
              <option v-for="type in this.master_data.types" v-bind:value="{ id: type.id, text: type.title }" :key="type.id">
                {{ type.title }}
              </option>
            </select>  
          </b-col>
          <b-col cols="4" v-for="key in filtered_keys" :key="key" class="mb-4">
            ...
            <div v-if="field_data[key].type !== 'option'">
...
export default {
  ...
  methods: {
    ...
    set_materdata_html: function () {
      ...
      this.master_data.types.forEach(function(type){
      ...
      let current_brand=this.data.brand
      let current_brand_info={}
      this.master_data.brands.forEach(function(type){
          if ( type.title == current_brand){
            current_brand_info=type
          }
      });
      this.selectedBrand = {
      ...
      this.master_data.modelsBasedOnBrand = this.master_data.models.filter(function(item)
      {
        return item.id_brand==current_brand_info.id;
      });
      let current_model=this.data.model
      let current_model_info={}
      this.master_data.modelsBasedOnBrand.forEach(function(model){
