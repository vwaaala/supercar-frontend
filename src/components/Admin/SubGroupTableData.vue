<template>
  <b-row class="justify-content-md-center">
    <b-modal ref="my-modal" size="lg" hide-footer title="Add Sub Category">
      <div >      
        <b-form >
          <b-form-group
            id="input-group-1"
...
export default {
  ...
  methods: {
    ...
    onArchive(item) {
      console.log(item)
      this.service.archive("sub-groups",item.id).then(() => {
    ...
    async get_data(page = 1, limit = 12, sort = 'id', q = '', order = 'desc') {
      ...
      let response = (await this.service.get_all_items("sub-groups",params)).data
    ...
    onUpdateItem(){
      ...
      this.service.update("sub-groups",this.form).then(response => {
    ...
    onSubmit() {
        ...
        this.service.create("sub-groups",this.form).then(response => {
  ...
  async created() {
    let response = (await this.service.get_all_items("sub-groups")).data
