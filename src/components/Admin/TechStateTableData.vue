<template>
  <b-row class="justify-content-md-center">
    <b-modal ref="my-modal" size="lg" hide-footer title="Add Technical Condition">
      <div >      
        <b-form >
          <b-form-group
            id="input-group-1"
...
export default {
  ...
  data: function () {
    return {
      ...
      config: {
        server_mode: true,
        card_title: "Technical Condition",
      ...
      actions: [
        {
          btn_text: "Add Technical Condition",
  ...
  methods: {
    ...
    onArchive(item) {
      console.log(item)
      this.service.archive("tech-state",item.id).then(() => {
    ...
    async get_data(page = 1, limit = 12, sort = 'id', q = '', order = 'desc') {
      ...
      let response = (await this.service.get_all_items("tech-state",params)).data
    ...
    onUpdateItem(){
      ...
      this.service.update("tech-state",this.form).then(response => {
    ...
    onSubmit() {
        ...
        this.service.create("tech-state",this.form).then(response => {
  ...
  async created() {
    let response = (await this.service.get_all_items("tech-state")).data
